...
const Cart = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      className: 'text-xs',
      render: (_, item) => (
        <div className='flex sm:flex-row flex-col items-center justify-evenly sm:w-full w-[30px]'>
          <button className='bg-gray-50' onClick={() => removeQuantity(item.id)}>-</button>
          <h1 className='sm:text-lg text-xs'>{item.quantity}</h1>
          <button className='bg-gray-50' onClick={() => addQuantity(item.id)}>+</button>
        </div>
      ),
    },
    {
      ...
      className: 'text-xs',
      render: (_, item) => (
        <h1 className='sm:text-lg text-xs sm:w-full w-[40px]'>{(item.price * item.quantity).toFixed(2)}</h1>
      ),
    },
    {
      title: 'Action',
      ...
      className: 'text-xs',
      render: (_, item) => (
          <button className='sm:text-lg text-xs text-red-500' onClick={() => removeFromCart(item.id)}><FontAwesomeIcon icon={faRemove} color='red' /></button>
      ),
    },
  ];
  ...
  return (
    <div>
      ...
      <div test-id="cartItem" className='lg:mx-36 mx-0 flex md:flex-row flex-col'>
        <Table dataSource={cartItems} columns={columns} />


        <div className='lg:mx-[20px] sm:mt-0 mt-12 py-[20px] px-[30px] w-[400px] h-[400px] bg-gray-100 rounded-sm'>
          <h1>Cart Total</h1>
          <hr className='mt-2' />
