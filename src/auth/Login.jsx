    ...
    return (
        <div className='background h-screen flex flex-col justify-center items-center gap-16'>
            ...
            <div className='auth w-[350px]  bg-[transparent] border-2 border-gray-600 rounded-xl flex flex-col justify-center items-start p-8'>
                <h1 className='text-gray-600 text-3xl'>Login</h1>
                <form className='flex flex-col gap-4 w-full mt-8 text-center'>
                    <input className='py-2 px-2 bg-transparent w-full text-white text-lg lg:placeholder:text-sm placeholder:text-lg placeholder:text-gray-700 outline-none focus:border-b-2 focus:border-gray-50 focus:text-xl focus:text-gray-800 caret-red-500 border-b-[1px] border-b-gray-500' required type="text" placeholder="Enter Email" name='email' onChange={handleChange} />
                    ...
                    <button className='mt-6 px-4 py-3 rounded-lg bg-[#FC4747] text-white text-sm hover:bg-white hover:text-[#161D2F] transition hover:ease-in-out' onClick={handleSubmit}>Login to your account</button>
                    {err && <p>{err}</p>}
                    <p className='mt-4 text-gray-700 text-sm'>Don't you have an account? <Link to="/auth/register"><span className='text-red-500 ms-2'>Sign Up</span></Link></p>
